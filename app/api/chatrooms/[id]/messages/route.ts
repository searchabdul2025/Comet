...
export async function POST(
  ...
  try {
    ...
    // Send to WhatsApp if the user is an Admin sending to a WhatsApp-linked user
    try {
      ...
      if (dbUser && dbUser.role === 'Admin') {
        if (chatroom.allowedUsers && chatroom.allowedUsers.length > 0) {
          ...
          if (targetUser && targetUser.whatsappNumber) {
            ...
            const phoneIdSetting = await Setting.findOne({ key: 'WHATSAPP_PHONE_NUMBER_ID' });
            if (tokenSetting?.value && phoneIdSetting?.value) {
              await fetch(`https://graph.facebook.com/v18.0/${phoneIdSetting.value}/messages`, {
                method: 'POST',
                headers: {
