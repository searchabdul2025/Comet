...
export function generateExcelWorkbook(options: ExcelExportOptions): XLSX.WorkBook {
  ...
  if (separateSheetsByAgent && agentNames && agentNames.size > 0) {
    // Group submissions by agent
    const submissionsByAgent = new Map<string, any[]>();
    
    // Also keep track of a "All Submissions" sheet if requested? The proposal says:
    // "Combined Excel or separate sheets per agent"
    // "Sheet 1: All Submissions ... Additional Sheets: One sheet per selected agent"
    
    // First, add the All Submissions sheet
    const allRows = submissionsToExcelRows(submissions, agentNames, formTitles);
    const allWorksheet = XLSX.utils.json_to_sheet(allRows);
    applyWorksheetFormatting(allWorksheet, allRows);
    XLSX.utils.book_append_sheet(workbook, allWorksheet, 'All Submissions');

    // Grouping
    submissions.forEach(sub => {
      const agentId = sub.submittedBy ? sub.submittedBy.toString() : 'Unknown';
      if (!submissionsByAgent.has(agentId)) {
        submissionsByAgent.set(agentId, []);
      }
      submissionsByAgent.get(agentId)!.push(sub);
    });

    // Add individual agent sheets
    submissionsByAgent.forEach((agentSubs, agentId) => {
      const agentName = agentNames.get(agentId) || 'Unknown';
      // Excel sheet names max 31 chars and no illegal chars
      const safeSheetName = agentName.replace(/[\\/?*\[\]:]/g, '').substring(0, 31);
      
      const rows = submissionsToExcelRows(agentSubs, agentNames, formTitles);
      if (rows.length > 0) {
        const worksheet = XLSX.utils.json_to_sheet(rows);
        applyWorksheetFormatting(worksheet, rows);
        
        // Ensure unique sheet name if there are collisions
